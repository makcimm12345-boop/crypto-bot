...
alerted = {}  # {coin_id: timestamp последнего алерта}
ALERT_COOLDOWN = 4 * 3600  # повторный алерт не раньше чем через 4 часа
...
def get_ai_analysis(symbol, name, price, pct_24h, market_cap, volume):
    prompt = (
        ...
        f"В конце на новой строке одно слово: ПОКУПАТЬ / ОСТОРОЖНО / ИЗБЕГАТЬ"
    )
    try:
        res = requests.post(
...
def scan():
    ...
    for coin in COINS:
        ...
        if pct_24h <= THRESHOLD:
            ...
            if "ПОКУПАТЬ" in last_line:
                emoji = "🟢"
            elif "ИЗБЕГАТЬ" in last_line:
                emoji = "🔴"
            else:
                emoji = "🟡"
